...
const MarkdownTable: React.FC<{ content: string; primaryColor: string }> = ({ content, primaryColor }) => {
  ...
  return (
    <div 
      className="my-8 overflow-x-auto rounded-lg border-2 break-inside-avoid page-break-inside-avoid"
      style={{ borderColor: primaryColor, boxShadow: `4px 4px 0px 0px ${primaryColor}` }}
    >
      <table className="w-full text-left text-sm border-collapse min-w-full table-auto">
        <thead style={{ backgroundColor: primaryColor }} className="text-white uppercase text-[10px] tracking-widest font-black">
          <tr>
            {headers.map((h, i) => (
              <th key={i} className="px-5 py-4 border-r border-white/20 last:border-0">{h}</th>
            ))}
          </tr>
        </thead>
...
const RenderBlock: React.FC<{ block: ContentBlock; primaryColor: string }> = ({ block, primaryColor }) => {
  ...
  switch (block.type) {
    ...
    case 'paragraph':
      return (
        <p className="text-slate-800 leading-[1.8] mb-8 academic-serif text-[1.15rem] text-justify break-words break-inside-avoid-page">
          {cleanedContent}
        </p>
      );
    case 'list':
    ...
    <div className="pb-32 overflow-x-hidden" style={{ counterReset: 'page-counter' }}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          /* Ensure text content doesn't get cut in half vertically */
